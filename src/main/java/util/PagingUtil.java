...
public class PagingUtil {
	...
	public static String pagingAjax(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page) {
		...
		if (intTemp != 1) {
			// 첫번째 페이지 블럭에서는 출력되지 않음
			// 두번째 페이지 블럭부터 출력됨.
			pagingStr += "" + "<a href='javascript:paging(1);'>" + "<img src='../images/paging1.gif'></a>";
			pagingStr += "&nbsp;";
			pagingStr += "" + "<a href='javascript:paging(" + (intTemp - blockPage) + ");'>"
					+ "<img src='../images/paging2.gif'></a>";
		}
		...
		while (blockCount <= blockPage && intTemp <= totalPage) {
			if (intTemp == nowPage) {
				...
				pagingStr += "&nbsp;<a href='javascript:paging(" + intTemp + ");'>" + intTemp + "</a>&nbsp;";
			}
			intTemp++;
			blockCount++;
		}
		...
		// 5.다음페이지블럭 & 마지막페이지 바로가기
		if (intTemp <= totalPage) {
			pagingStr += "<a href='javascript:paging(" + intTemp + ");'>" + "<img src='../images/paging3.gif'></a>";
			pagingStr += "&nbsp;";
			pagingStr += "<a href='javascript:paging(" + totalPage + ");'>" + "<img src='../images/paging4.gif'></a>";
		}
